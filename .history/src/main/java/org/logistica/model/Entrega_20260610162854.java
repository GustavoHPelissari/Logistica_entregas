...
public class Entrega {
    ...
    public void concluirEntrega() {
        if (this.statusEntrega == StatusEntrega.CANCELADA) {
            throw new RuntimeException("Não é possível concluir uma entrega cancelada");
        }
    ...
    public void cancelarEntrega() {
        if (this.statusEntrega == StatusEntrega.ENTREGUE) {
            throw new RuntimeException("Não é possível cancelar uma entrega já concluída");
        }
        this.statusEntrega = StatusEntrega.CANCELADA;
    }

    public void marcarComoAtrasada(){
        if (this.statusEntrega == StatusEntrega.EM_ROTA) {
            this.statusEntrega = StatusEntrega.ATRASADA;
        } else{
            throw new RuntimeException("Não é possível marcar um entrega que não está em rota como atrasada")
        }
    }
